/**
 * Settings — JVML Stock config hook return contract.
 *
 * `JvmlStockConfigCard` takes NO props (FE-9: it owns `useJvmlStockConfig()`
 ...
 *
 * SPEC: .planning/jvml-stock/SPEC.md §3 types/settings/jvml-stock-config-ui.ts
...
import type {
  ...
  JvmlStockStatus,
} from "./jvml-stock-config"

// ---------------------------------------------------------------------------
// useJvmlStockConfig hook return contract
// ---------------------------------------------------------------------------

/**
 * Return type of `useJvmlStockConfig` — used internally by `JvmlStockConfigCard`.
 *
...
export interface UseJvmlStockConfigResult {
  ...
  isLoading: boolean
  /**
   * True while the PUT /admin/jvml-stock/config request is in-flight.
   * The "Save configuration" button shows a spinner and is disabled while true.
   */
  isSaving: boolean
  /**
   * True while the POST /admin/jvml-stock/run-now request is in-flight.
   ...
   */
  save: (input: JvmlStockConfigInput) => Promise<void>
  /**
   * Trigger an immediate poll via POST /admin/jvml-stock/run-now.
