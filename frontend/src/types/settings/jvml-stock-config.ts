/**
 * Settings — JVML Stock Excel config domain types.
 *
 * Mirrors backend schemas:
 *   - JvmlStockConfigPublic  → JvmlStockConfig
 *   - JvmlStockConfigUpdate  → JvmlStockConfigInput
 *   - JvmlStockStatusPublic  → JvmlStockStatus
 *   - JvmlStockIngestionRow  → JvmlStockIngestionRow
 *
 * SPEC: .planning/jvml-stock/SPEC.md §3 types/settings/jvml-stock-config.ts
 * ADDENDUM: A-FE clarifications (FE-9: card owns hook; no id on config).
 */

// ---------------------------------------------------------------------------
// Config — singleton document returned by GET /admin/jvml-stock/config
// ---------------------------------------------------------------------------

/**
 * The JVML Stock Excel ingestion configuration singleton.
 * Backend: JvmlStockConfigPublic — no `id` field (BE-17: intentional omission).
 */
export interface JvmlStockConfig {
  ...
  /**
   * Addresses that receive the "JVML STOCK EXCEL not found" alert email.
   * Stored lowercase and deduplicated by the backend.
   */
  notify_emails: string[]
  /**
   * ISO-8601 datetime of the last PUT /admin/jvml-stock/config save,
...
// ---------------------------------------------------------------------------
// Config input — PUT /admin/jvml-stock/config request body
// ---------------------------------------------------------------------------

/**
 * Form submission shape for saving the JVML Stock config.
 * Identical to JvmlStockConfig minus `updated_at` (set by the server).
 */
...
/**
 * One entry in the JvmlStockStatus.recent array.
 * Mirrors the backend JvmlStockIngestionRow schema.
 */
...
// ---------------------------------------------------------------------------
// Status — GET /admin/jvml-stock/status response
// ---------------------------------------------------------------------------

/**
 * Live ingestion status for the JVML Stock poll job.
 * Mirrors the backend JvmlStockStatusPublic schema.
 */
