/**
 * Settings — JSW Stock config hook return contract.
 *
 * `JswStockConfigCard` takes NO props (FE-9: it owns `useJswStockConfig()`
...
import type {
  ...
  JswStockConfigInput,
  JswStockStatus,
} from "./jsw-stock-config"

// ---------------------------------------------------------------------------
// useJswStockConfig hook return contract
// ---------------------------------------------------------------------------

/**
 * Return type of `useJswStockConfig` — used internally by `JswStockConfigCard`.
 *
...
export interface UseJswStockConfigResult {
  ...
  isLoading: boolean
  /**
   * True while the PUT /admin/jsw-stock/config request is in-flight.
   * The "Save configuration" button shows a spinner and is disabled while true.
   */
  isSaving: boolean
  /**
   * True while the POST /admin/jsw-stock/run-now request is in-flight.
  ...
  /**
   * Trigger an immediate poll via POST /admin/jsw-stock/run-now.
