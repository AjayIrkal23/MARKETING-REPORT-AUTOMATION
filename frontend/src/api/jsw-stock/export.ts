/**
 * `GET /jsw-stock/export` — download matching rows as .xlsx.
 *
...
export async function exportJswStock(
  ...
  const url = `/jsw-stock/export${queryString ? `?${queryString}` : ""}`
  const defaultFilename = `jsw_stock_${query.date || "all"}.xlsx`
