# BE implementation note: `app/utils/jsw_stock/columns.py`

**Area:** columns.py — the single source of truth for the 72-column Excel→Mongo mapping.

**Validation status:** ALL 72 Excel header strings verified against `macro_docs/zsd-currstk-hr.md`
summary table (exact casing, spacing, punctuation). Zero mismatches found.

Notable casing/spacing traps confirmed correct in SPEC §1:
- Col 3: `"Distr.Chnl"` (abbreviated, period after "Distr", not "Distr. Chnl")
- Col 12: `"Act.Thickness (mm)"` (period after "Act", space before "(mm)")
- Col 22: `"Length(mm)"` (NO space before "(mm)" — differs from col 12 and 13)
- Col 38: `"UNLOADING POINT"` (ALL CAPS)
- Col 39: `"RECIEVING POINT"` (ALL CAPS, sic misspelling — this IS the real header)
- Col 45: `"production workcenter"` (fully lowercase — this IS the real header)
- Col 64: `"Tensile Strength MPa (B)"` (mixed case with trailing `(B)`)

**Line count:** The full file below is ~235 lines — safely under 250.

---

## Full copy-pasteable file content

```python
# app/utils/jsw_stock/columns.py
"""
Column map for ZSD_CURRSTK_HR.xlsx  (72 columns).

Single source of truth. Used by:
  - excel.py  (HEADER_TO_FIELD for header→field mapping)
  - ingest.py (coerce_value for type coercion)
  - models/jsw_stock.py (field list reference)
  - schemas/jsw_stock.py (filter/sort field validation)
...
## Corrections / clarifications vs SPEC §1

None. Every header string in SPEC §1 matches the macro_doc summary table verbatim.
Field names, type tags, and the 3-tuple ordering are correct.

## Structural notes for the builder

1. The `assert len(COLUMNS) == 72` guard will catch any accidental add/remove during edits.
2. `HEADER_TO_FIELD` uses `normalize_header` (collapse whitespace → lower) so the excel.py
   parser can tolerate minor whitespace variation in actual row-1 cells (e.g. trailing spaces
   on some SAP column headers noted in `CLAUDE.md`).
3. `coerce_value` handles the `"1.057.000"` width cell: `parse_workbook` will return it as a
   raw string (not float) because it fails `float()` — and the `text` branch just strips it.
4. `LC Exp Date` is `text` (dd.mm.yyyy stored as text in SAP) — NOT `date`. Do not change this.
5. `ELONGATION` (col 47) is `text` even though it looks numeric — matches macro_doc (0.1% fill,
   stored as text in the source workbook). `Elongation(Mic)` (col 48) is `number`.
6. `YIELD STRENGTH` (col 65) and `UTS` (col 66) are `text` — confirmed by macro_doc "STORED AS TEXT".
7. `HARDNESS` (col 49) is `text` — confirmed by macro_doc "STORED AS TEXT, leading-space dirty values".
   The `text` branch `.strip()` will clean leading/trailing spaces automatically.
8. File is self-contained (stdlib only: `re`, `datetime`). No project imports needed.
