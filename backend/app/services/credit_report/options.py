"""CreditReport domain: async combobox field-options search.
...
Beanie 2.1.0 API note: ``CreditReport.distinct(field, filter_dict)`` accepts the
...
CreditReportField whitelist has 4 fields:
...
from ...schemas.credit_report import CreditReportOptionsQuery
...
async def search_field_options(
    query: CreditReportOptionsQuery,
) -> list[AsyncOption]:
    """Return up to ``query.limit`` distinct field values as combobox options.

    Calls ``CreditReport.distinct(field, filter)`` — a Beanie Document classmethod
    ...
    Args:
        query: ``CreditReportOptionsQuery`` containing:
               - ``field``: one of the four ``CreditReportField`` literals
    ...
    # CreditReport.distinct(query.field, filter=filt) -> TypeError (wrong)
    # CreditReport.distinct(query.field, filt)        -> correct
    values: list[Any] = await CreditReport.distinct(query.field, filt)
