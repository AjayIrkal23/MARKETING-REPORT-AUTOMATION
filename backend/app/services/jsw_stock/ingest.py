...
Flow:
  ...
  4. Build JswStock documents — ONLY rows passing should_keep_row
  ...
  6. Emit ``jsw_stock.ingested`` audit event.
...
async def ingest_file(path: str, report_date: str) -> int:
    """Parse *path*, map customer codes, and bulk-insert into ``jsw_stock``.
    ...
    Inserts in chunks of 1 000 (BE-9) so the BSON wire size stays bounded
    for large reports (~17 000 rows × 79 fields).
    ...
    # ── 3. Idempotent delete (BE-10: single await is correct) ─────────────────
    await JswStock.find({"report_date": report_date}).delete()

    # ── 4. Build JswStock documents (only rows passing the ingestion gate) ────
    now = _now_utc()
    docs: list[JswStock] = []
