"""Email rendering and delivery for the JSW Stock Excel missing-file alert.
...
Audit action emitted: ``jsw_stock.missing_alert`` (category ``jsw_stock``).
"""
    ...
    html = textwrap.dedent(f"""\
        ...
        <body style="font-family:sans-serif;color:#1a1a2e;max-width:520px;margin:auto;padding:24px">
          ...
          <p style="color:#94a3b8;font-size:.75rem">
            This alert is sent once per day when the JSW Stock Excel file is still
...
async def send_missing_alert(config: object, report_date: str) -> None:
    ...
    * Emits ``jsw_stock.missing_alert`` audit event after the loop (success outcome
    ...
    * Logs at INFO when recipients exist; WARN when ``notify_emails`` is empty.
    ...
    Args:
        config:      The ``JswStockConfig`` document (or any object exposing
    ...
    from ..audit.events import audit_jsw_stock_event  # noqa: PLC0415
