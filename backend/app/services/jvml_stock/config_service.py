"""JVML Stock Config service — singleton get and upsert.
...
Circular-import avoidance (B-9 / BE-15):
    ``scheduler.py → services.cron.jvml_stock → services.jvml_stock.poller →
    services.jvml_stock.config_service → scheduler.py``

    ``apply_jvml_stock_schedule`` is therefore imported **locally** inside
...
def _to_public(doc: JvmlStockConfig) -> JvmlStockConfigPublic:
    """Map a :class:`JvmlStockConfig` document to the public DTO.
...
async def get_config() -> JvmlStockConfigPublic:
    """Return the singleton JVML Stock config, or schema defaults if absent.
    ...
    Returns:
        :class:`JvmlStockConfigPublic` — either from DB or hard-coded defaults.
    """
...
async def upsert_config(
    ...
    *,
    ...
    - After saving: call :func:`apply_jvml_stock_schedule` (local import to
      break the scheduler ↔ service circular dependency — BE-15 / B-9).
    - Emit ``"jvml_stock.config_updated"`` audit event with ``actor_email`` and
      a compact ``extra`` payload (enabled flag, interval, email count).

    Args:
        body:        Validated PUT body from :class:`JvmlStockConfigUpdate`.
        actor_email: Admin's email address threaded from the request dependency;
                     ``None`` if unavailable.

    Returns:
        :class:`JvmlStockConfigPublic` reflecting the saved state.
    """
    # Local import avoids circular dependency:
    #   scheduler.py → services.cron.jvml_stock → services.jvml_stock.poller
    #   → services.jvml_stock.config_service → scheduler.py  ← cycle!
    ...
    # Emit structured audit event — category="jvml_stock" (BE-13: singular).
    await audit_jvml_stock_event(
        "jvml_stock.config_updated",
        "JVML Stock Excel config updated",
        outcome="success",
