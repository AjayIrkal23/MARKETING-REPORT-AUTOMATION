"""Email rendering and delivery for the JVML Stock Excel missing-file alert.
...
Audit action emitted: ``jvml_stock.missing_alert`` (category ``jvml_stock``).
"""
...
def render_missing_email(
    ...
    text = textwrap.dedent(f"""\
        The JVML Stock Excel file was not found by the end of the monitoring window.
    ...
    html = textwrap.dedent(f"""\
        ...
        <body style="font-family:sans-serif;color:#1a1a2e;max-width:520px;margin:auto;padding:24px">
          ...
          <p style="color:#94a3b8;font-size:.75rem">
            This alert is sent once per day when the JVML Stock Excel file is still
...
async def send_missing_alert(config: object, report_date: str) -> None:
    ...
    * Emits ``jvml_stock.missing_alert`` audit event after the loop (success outcome
    ...
    * Logs at INFO when recipients exist; WARN when ``notify_emails`` is empty.
    ...
    Args:
        config:      The ``JvmlStockConfig`` document (or any object exposing
    ...
    from ..audit.events import audit_jvml_stock_event  # noqa: PLC0415
