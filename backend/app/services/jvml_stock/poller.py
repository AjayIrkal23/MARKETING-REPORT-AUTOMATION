"""JVML Stock Excel daily poll — file detection, ingest dispatch, and alert.

Entry point: ``run_poll() -> JvmlStockStatusPublic``.
...
- Ingestion is idempotent: ``JvmlStock.find(report_date=...).delete()`` runs first
...
- Any exception during ingest → mark ``"error"`` + emit ``jvml_stock.failed`` audit.
...
def _parse_hhmm(s: str) -> dt_time:
    ...
    Caller guarantees the format is valid (validated by ``JvmlStockConfigUpdate``).
    """
    h, m = map(int, s.split(":"))
    return dt_time(h, m)
...
async def run_poll() -> JvmlStockStatusPublic:
    """Detect and ingest today's JVML Stock Excel file.

    Returns the current :class:`JvmlStockStatusPublic` regardless of outcome so
    ...
    try:
        ...
        # Re-ingest on EVERY in-window poll tick — no skip-once guard. Each run is
        # a snapshot refresh of today's rows (ingest_file deletes the date then
        # bulk re-inserts), so new party codes / items get picked up and
        # duplicates are impossible. `last_run_at` is stamped every tick so the
        # dashboard/status UI can show the poll is live (persisted on save below).
        ...
        if fpath is not None:
            ...
            try:
                ...
                from ...services.audit.events import audit_jvml_stock_event  # noqa: PLC0415
