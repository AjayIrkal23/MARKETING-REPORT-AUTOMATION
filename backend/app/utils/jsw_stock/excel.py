"""JSW Stock workbook parser — thin wrapper over the shared parser.

Container detection (.xlsx/.xlsm/.xlsb) and row extraction live in
``app.utils.shared.excel``; this module only binds the JSW column map so the
...
def parse_workbook(data: bytes) -> list[dict[str, Any]]:
    """Parse JSW Stock workbook bytes into raw ``[{field: value}]`` dicts."""
