# app/utils/jvml_stock/query.py
"""Query helpers for the jvml_stock domain (filter + sort building).

Mirrors utils/jsw_stock/query.py. Filter surface (JSW/JVML stock filter
overhaul, 2026-06): exactly 5 filters — a single ``date`` (exact match on the
stored ``report_date`` string) plus four per-field exact-match filters. No
free-text ``q``, no created_at range, so no ``$regex``.
"""
...
# 5 fields exposed as per-field exact-match filter params (JvmlStockField order).
...
def build_sort(sort_by: str, sort_order: str) -> str:
    ...
    ``sort_by`` is constrained to ``JvmlStockSortBy`` at the schema layer so only
