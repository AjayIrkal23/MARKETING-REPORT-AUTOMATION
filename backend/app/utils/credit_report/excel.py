"""Credit Report workbook parser — thin wrapper over the shared parser.

Container detection (.xlsx/.xlsm/.xlsb) and row extraction live in
``app.utils.shared.excel``; this module only binds the credit-report column map
so the shared parser can label columns. Callers apply ``coerce_value`` from
...
def parse_workbook(data: bytes) -> list[dict[str, Any]]:
    """Parse Credit Report workbook bytes into raw ``[{field: value}]`` dicts."""
