"""JSW Stock Config / Status DTOs.

Schema contract for:
  - GET /admin/jsw-stock/config  → JswStockConfigPublic
  - PUT /admin/jsw-stock/config  → JswStockConfigUpdate (request body)
  - GET /admin/jsw-stock/status  → JswStockStatusPublic (embeds JswStockIngestionRow)

BE-17: JswStockConfigPublic has NO id field (singleton config, key never exposed).
...
GAP-1: model_validator(mode="after") on JswStockConfigUpdate is REQUIRED per plan-check.
"""
...
class JswStockConfigUpdate(BaseModel):
    """PUT body for PUT /admin/jsw-stock/config.
    ...
    def normalize_emails(cls, v: Any) -> list[str]:
        """Lowercase, strip, deduplicate (preserving first-seen order).
...
# ---------------------------------------------------------------------------
# Ingestion row — nested inside JswStockStatusPublic.recent
# ---------------------------------------------------------------------------
