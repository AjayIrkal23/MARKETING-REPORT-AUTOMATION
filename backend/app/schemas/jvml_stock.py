"""JVML Stock request/response DTOs — list, options, and sort/filter literals.

Mirrors schemas/jsw_stock.py pattern (jsw -> jvml rename).
...
# SPEC §1 JvmlStockSortBy list, exact order; default: created_at
# ---------------------------------------------------------------------------
...
# Used by JvmlStockOptionsQuery and JvmlStockListQuery per-field filters.
...
class JvmlStockListQuery(PageQuery):
    ...
    Exactly 6 filters:
      ...
      - 5 per-field exact-match filters (one per JvmlStockField, None = off).
    """
    ...
    # 5 per-field exact-match filters (one per JvmlStockField, None = no filter).
