"""Credit Report Config / Status DTOs.

Schema contract for:
  - GET /admin/credit-report/config  → CreditReportConfigPublic
  - PUT /admin/credit-report/config  → CreditReportConfigUpdate (request body)
  - GET /admin/credit-report/status  → CreditReportStatusPublic (embeds CreditReportIngestionRow)

BE-17: CreditReportConfigPublic has NO id field (singleton config, key never exposed).
...
GAP-1: model_validator(mode="after") on CreditReportConfigUpdate is REQUIRED per plan-check.
"""
...
class CreditReportConfigPublic(BaseModel):
    """Client-safe read projection of the Credit Report config singleton.
...
class CreditReportConfigUpdate(BaseModel):
    """PUT body for PUT /admin/credit-report/config.
    ...
    def check_start_before_end(self) -> "CreditReportConfigUpdate":
...
# ---------------------------------------------------------------------------
# Ingestion row — nested inside CreditReportStatusPublic.recent
# ---------------------------------------------------------------------------


class CreditReportIngestionRow(BaseModel):
...
class CreditReportStatusPublic(BaseModel):
    """Response shape for GET /admin/credit-report/status.
