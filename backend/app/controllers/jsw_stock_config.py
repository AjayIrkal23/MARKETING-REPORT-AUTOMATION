...
All four endpoints are gated by get_current_admin (admin users only).
  ...
  - get_config_controller:     GET  /admin/jsw-stock/config
  - update_config_controller:  PUT  /admin/jsw-stock/config
  - get_status_controller:     GET  /admin/jsw-stock/status
  - run_now_controller:        POST /admin/jsw-stock/run-now

Contract: SPEC.md §2 controllers + notes/be-routes-audit-wiring.md §2.2.
...
async def get_config_controller(
    ...
    """``GET /admin/jsw-stock/config`` — retrieve the current config singleton.
...
async def update_config_controller(
    ...
    """``PUT /admin/jsw-stock/config`` — replace the config and reschedule the job.
    ...
    (if disabled).  Emits ``jsw_stock.config_updated`` audit event.
    """
...
async def get_status_controller(
    ...
    """``GET /admin/jsw-stock/status`` — ingestion status and recent run history.
...
async def run_now_controller(
    ...
    """``POST /admin/jsw-stock/run-now`` — trigger an immediate stock file poll.
    ...
    """
    from ..services.jsw_stock.poller import run_poll  # noqa: PLC0415 — avoid cycle if poller imports scheduler
...
async def cleanup_duplicates_controller(
    ...
    """``POST /admin/jsw-stock/cleanup-duplicates`` — remove older same-date duplicates."""
    deleted = await cleanup_duplicates(JswStock, report_date)
    return success(
